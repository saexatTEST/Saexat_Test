...
function normalizeBookings(input: unknown): Booking[] {
  ...
  for (const item of input) {
    ...
    if (!b.id || !b.roomNumber || !b.checkIn || !b.checkOut || !b.status) {
      continue;
    }
  ...
  for (const b of byId.values()) {
    bySignature.set(bookingSignature(b), b);
  }
...
function bookingsConflict(a: Booking, b: Booking): boolean {
  ...
  if (!eitherIsRoomWide) {
    const aBeds = new Set<number>([
      a.bedIndex as number,
      ...(a.additionalBeds ?? []),
    ]);
    const bBeds = new Set<number>([
      b.bedIndex as number,
      ...(b.additionalBeds ?? []),
    ]);
  ...
  useEffect(() => {
    ...
    const loadCloud = async () => {
      try {
        ...
        // keep local state if cloud is temporarily unavailable
      }
    };
